...
class TestModelSimulator(unittest.TestCase):
    ...
    def test_value_method_of_model(self):
        ...
        self.assertEqual("Non existing",
                         ...
                         msg="파일 확인용, 파일 자체가 없으므로 없다고 해야 함")
        FileChecker(**info_cfx_status).write_file(self.test_aiosdata.cfx_status) #CFX_status.csv 생성
        FileChecker(**info_plate_exist).write_file(self.test_aiosdata.plate_exist)
        FileChecker(**info_elevator_enable).write_file(self.test_aiosdata.elevator_enable)
        self.model_simulator.file_data = self.test_aiosdata #Method_status.csv, test.trc 생성
